const UNLOCK_KEY = "waymark-library-unlocked";
...
function actionLabel(book, unlocked) {
  if (unlocked) return book.ctaUnlocked || "Download";
  return book.cta || "Get free ebook";
}

function renderBooks(books) {
  if (!books.length) {
    els.grid.innerHTML = "<p class=\"loading\">No guides yet.</p>";
    return;
  }
...
function showSuccess(book) {
  ...
  if (isExternalUrl(book.file)) {
    els.gateDownload.removeAttribute("download");
    els.gateDownload.target = "_blank";
    els.gateDownload.rel = "noopener";
  } else {
    els.gateDownload.setAttribute("download", fileName(book.file));
    els.gateDownload.removeAttribute("target");
  }
...
function openGate(book) {
  ...
  els.gateExternal.href = book.formShareUrl || catalog.gate.formShareUrl;
...
els.gateForm.addEventListener("load", () => {
  // Embedded form: first load = form, second load ≈ confirmation after submit.
  formLoadCount += 1;
